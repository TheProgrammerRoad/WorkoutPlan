...
//    }
    ...
    public void putListTrainingNames(String key, ArrayList<String> trainingNames){
        checkForNullKey(key);
        Gson gson = new Gson();
        ArrayList<String> objStrings = new ArrayList<String>();
        for(String trainingName : trainingNames){
            objStrings.add(gson.toJson(trainingName));
        }
        putListString(key, objStrings);
    }

    //No need Class<?> mClass parameter. Because we know it is Player!
    public ArrayList<String> getListTrainingNames(String key){
        Gson gson = new Gson();
        ...
        for(String jObjString : objStrings){
            String trainingName  = gson.fromJson(jObjString,  String.class);
            playerList.add(trainingName);
        }
        return playerList;
    }
}